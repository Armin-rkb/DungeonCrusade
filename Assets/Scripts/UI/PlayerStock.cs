...
public class PlayerStock : MonoBehaviour {
    ...
    void BreakStock()
    {
        if (_playerDeath.GetDeath && _playerDeath.playernum == 1)
        {
            _pStocks[0].sprite = _breakStock;
        }

        else if (_playerDeath.GetDeath && _playerDeath.playernum == 2)
        {
            _pStocks[1].sprite = _breakStock;
        }

        else if (_playerDeath.GetDeath && _playerDeath.playernum == 3)
        {
            print("P3 IZ DED");
            _pStocks[2].sprite = _breakStock;
        }

        else if (_playerDeath.GetDeath && _playerDeath.playernum == 4)
        {
